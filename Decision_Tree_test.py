...
def create_tree():
    ...
    male_left = TreeNode('Survived')
    male_right = TreeNode('Died')
...
def is_survived(sex, age, decision_tree):
    r = DFS_tree(decision_tree, 0, sex, age)
    return r


def DFS_tree(node, deep, sex, age):
    r = 0
    if node.val == 'Survived':
        ...
        result = is_male(sex)
        if result:
            r =DFS_tree(node.left, (deep + 1), sex, age)
        else:
            r =DFS_tree(node.right, (deep + 1), sex, age)
    elif node.val == 'Age':
        result = is_baby(age)
        if result:
            r = DFS_tree(node.left, (deep + 1), sex, age)
    ...
    with open('C:/evillog/Machine_Learning/Titanic/test.csv', 'r') as testfile:
        ...
        for row in reader:
            ...
            if row['Age'] == "":
                ...
                age = float(row['Age'])
            result = is_survived(sex, age, decision_tree)
